...
def upload_directory(ftp, local_dir, remote_dir):
    ...
    ensure_remote_dir_exists(ftp, remote_dir)  # Ensure the root remote directory exists
...
def upload_folder_to_ftp(host, port, username, password, local_folder, remote_folder):
    """Main function to upload a folder to the FTP server."""
    try:
        ftp = connect_to_ftp(host, port, username, password)  # Connect to the FTP server
        upload_directory(ftp, local_folder, remote_folder)  # Upload the folder
        ftp.quit()
...
# Example usage
host = "localhost"  # Replace with your FTP server's hostname or IP
port = 21           # Replace with your FTP server's port number
username = "user1"  # Replace with your FTP username
password = "password1"  # Replace with your FTP password
local_folder = "/path/to/local/folder"  # Replace with the local folder path
remote_folder = "/remote/folder"        # Replace with the remote folder path

upload_folder_to_ftp(host, port, username, password, local_folder, remote_folder)
