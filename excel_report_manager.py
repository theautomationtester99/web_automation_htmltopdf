...
class ExcelReportManager:
    ...
    def __init__(self, logger, lock):
        ...
        self.all_skipped_test_results_list = []
    ...
    def add_row_skipped_tc(self, test_result):
        """
        Adds a new test result row to the list and updates the Excel file.
        ...
        """
        with self.lock:
            if self.utils.check_if_file_exists(os.path.join(self.result_folder, "skipped_tc_report.xlsx")):
                df = pd.read_excel(os.path.join(self.result_folder, "skipped_tc_report.xlsx"))
                self.all_skipped_test_results_list = df.values.tolist()
                self.all_skipped_test_results_list.append(test_result)
                self.export_to_excel_skipped_tc()
            else:
                self.all_skipped_test_results_list.append(test_result)
                self.export_to_excel_skipped_tc()
    ...
    def export_to_excel_skipped_tc(self):
        """
        Exports the list of test results to an Excel file ('output.xlsx').
        ...
        df = pd.DataFrame(self.all_skipped_test_results_list, columns=['test_script', 'why_skipped', 'Status'])
        data_condition = ['Failed', 'Fail', 'FAILED']
        ...
        # print(type(styler))
        # print(pd.__version__)
        output_file = os.path.join(self.result_folder, "skipped_tc_report.xlsx")
        styler.to_excel(output_file, index=False)
